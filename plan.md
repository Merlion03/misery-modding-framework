...
> **Исторический документ. Phase 1 закрыта.**
>
> Это утверждённый план **исследовательской фазы**: её задачи, milestone-ы
> M0–M9 и exit criteria — то есть то, что делалось до появления платформы. Он
> **не описывает текущее состояние проекта**: с тех пор закрыты Stage 2, 3, 4,
> 4.5, 5A, 5B, 7 и 8, и production loader, `MiseryRuntime`, CoreCLR-хост и
> публичный C#-API существуют, хотя §6 ниже относит их «вне области этого
> документа».
>
> **Текущее состояние — только `README.md`.** Здесь ничего не переписано задним
> числом: план ценен ровно тем, что фиксирует, что и почему планировалось тогда.
...
| ID | Вопрос | Статус |
|---|---|---|
| Q-8.1 | Язык/технология ERI | **ANSWERED в M3** (~~UNKNOWN~~): **Python + `ctypes`**, вызывающий Toolhelp32/`OpenProcess`-семейство Win32 API напрямую. Rust и C++ (другие названные кандидаты) не выбраны по тому же критерию, каким выбран Python — «минимальная стоимость до первого дампа» (не будущая производительность инструмента, который по plan.md §8.1 по определению одноразовый). Источник: `research/instruments/eri/README.md` §«Технология и почему выбрана», `research/instruments/eri/eri.py` (капабилити I-01), `LOG-0047` |
