...
> MSVC/CRT 20, Unreal Engine 11, неатрибутированных 2, **классов игры 0**; ни одного имени
> `U*`/`A*` в файле нет вовсе; покрытие полиморфных классов ≤ 8,0 %, 4 428 из 411 385
...
| **A-09h** | Состав **активных** модулей сборки: какие из 250 staged-плагинов включены в сборку — неизвестно. Предположение «staged-набор примерно и есть включённый набор» остаётся HYPOTHESIS | сопоставление staged-имён с ожидаемым составом сборки UE; проверки кодом не делалось | `container-metadata` | HYPOTHESIS | 0.4 | I | Уровень HYPOTHESIS делает класс I обязательным. 0.4 — то же значение, что в `research/unknowns.md` A-09 и CL-09; оно ниже порога 0.80, поэтому двух методов не требуется, и поднимать нечем: `container-metadata` доказывает только факт staged-файла. Ответ — M2: `binary-analysis` плюс `global-ucas`. Правило «staged ≠ enabled» обязательно писать в артефакте явно |
...
| Список 250 имён `*.uplugin` из индекса пакета | A-09b | `research/packages/plugins.json`, exit criterion M2 (3); пересчитать своим парсером, а не `strings` |
...
| A-09b | 0.75 | 0.75 | (c) | То же, что A-08: `strings` по индексу пакета — чтение без адреса. Число 250 никем не перепроверено, второго метода нет |
