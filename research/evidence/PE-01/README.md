# PE-01 — `UObject::ProcessEvent`: vtable-слот (HYPOTHESIS), адрес этой волной — UNKNOWN

> **ОБНОВЛЕНИЕ (2026-08-27, `research/RESEARCH_LOG.md` LOG-0056, capability PE-02):** адрес
> `ProcessEvent` этой (статической) волной не найден — см. ниже, честно и по-прежнему верно для
> МЕТОДОВ, доступных без runtime. Он НАЙДЕН отдельной, более поздней волной, живым runtime-чтением
> vtable-слота 77 у реальных объектов (`--run-pe02-vtable-scan`) плюс статической декомпиляцией
> результата: **`UObject::ProcessEvent` = live RVA `0x12ac1f0`** (`AActor::ProcessEvent`, override,
> = live RVA `0x321a430`), OBSERVED, class I, confidence 0.90 — два независимых метода
> (`runtime-reflection` + `binary-analysis`), слот 77 подтверждён напрямую (не только через
> `UEngine::Init`-перекрёстную проверку ниже). Эта страница НЕ переписана (append-only, C-10) — она
> остаётся точным отчётом о том, что было испробовано и честно не найдено БЕЗ доступа к runtime;
> подробности — `LOG-0056`, `research/instruments/eri/eri.py` (capability PE-02).
...
35576357. Основная зацепка задания — vtable-слот, а не строка. Итог ЭТОЙ (статической) волны:
**слот вычислен и независимо перекрёстно проверен (HYPOTHESIS, class I); конкретный адрес функции —
не найден СТАТИЧЕСКИ, отчитывается как UNKNOWN с изложением того, что испробовано (найден позже,
живым runtime-чтением — см. обновление выше).** Это ЗАКОННЫЙ результат по правилу 8 задания волны, а
не отказ от попытки — цепочка метода пройдена полностью, опровержение собственных кандидатов
выполнено честно, а не подменено слабой находкой.
