...
SRC = "D:/UEScratch/MBPLKit/Source_PNG/arm4"
PARENT_PATH = "/Game/PlayerElectricitySystem/Materials/M_BasicMaterial"
DIR = "/Game/MBPLProbe4"
MESH_NAME = "SM_Probe4"
MESH_PATH = "%s/%s" % (DIR, MESH_NAME)

ORDER = ["REF", "A", "B", "EM", "CTRL"]
SLOT_NAMES = ["M_P4_REF", "M_P4_A", "M_P4_B", "M_P4_EM", "M_P4_CTRL"]
...
try:
    ...
    task.filename = "%s/probe4.glb" % SRC
    ...
    bc_red = imp_tex("T4_BC_Red.png", "T4_BC_Red", True,
                     unreal.TextureCompressionSettings.TC_DEFAULT)
    bc_black = imp_tex("T4_BC_Black.png", "T4_BC_Black", True,
                       unreal.TextureCompressionSettings.TC_DEFAULT)
    nrm = imp_tex("T4_N.png", "T4_N", False,
                  unreal.TextureCompressionSettings.TC_NORMALMAP)
    arm = {k: imp_tex("T4_ARM_%s.png" % k, "T4_ARM_%s" % k, False,
                      unreal.TextureCompressionSettings.TC_MASKS)
           for k in ("REF", "A", "B", "DARK")}

    # EM and CTRL are IDENTICAL except that CTRL carries no emissive override.
    # That pairing is what separates a real glow from a specular highlight: a
    # highlight would appear on both.
    intended_tex = {
        ...
        "EM": {"BaseColor": bc_black, "ARM": arm["DARK"], "Normal": nrm},
        "CTRL": {"BaseColor": bc_black, "ARM": arm["DARK"], "Normal": nrm},
    }
    mics = {}
    for k in ORDER:
        extra = {}
        if k == "EM":
            extra = {"vectors": {"Emissive": (1.0, 0.35, 0.08),
                                 "EmissiveColor": (1.0, 0.35, 0.08)},
                     "scalars": {"EmissiveStrength": 50.0}}
        mics[k] = make_mic("MI_P4_%s" % k, parent, intended_tex[k], **extra)
...
if not os.path.isdir(OUT):
    os.makedirs(OUT)
with open(os.path.join(OUT, "probe4.json"), "w") as handle:
    json.dump(report, handle, indent=2, sort_keys=True)
unreal.log("[MBPL] wrote probe4.json  ok=%s" % report.get("ok"))
