...
свойств — выполнено: найден во всех четырёх запусках, прочитано девять свойств.

Требование M4 «способ доступа описан как структурное отношение» — выполнено (§5).

## Implications for future SDK

- Ввод, HUD и камера достижимы от контроллера. Контроллер **пересоздаётся при смене карты**, но
  **переживает смерть и возрождение** персонажа — измерено по `FUObjectItem::SerialNumber`
  (idx 76535, ser 21658 без изменений). Это две разные границы жизни, и путать их нельзя:
  ссылку нельзя держать через смену карты, но через возрождение — можно.
