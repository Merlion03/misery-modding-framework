...
### 4. Время жизни — смерть и возрождение измерены
...
**Возрождение наблюдалось.** Раннер по-прежнему fail-closed на экране смерти и не нажимает
«ВОЗРОДИТЬСЯ В БУНКЕРЕ» — это игровое решение, меняющее сейв. Кнопку нажал человек, а
`m4_respawn.py` записал переход: медленный полный проход до и после, и опрос трёх указателей
(`LocalPlayer → PlayerController → Pawn / AcknowledgedPawn`) каждые 50 мс между ними. Опрос
привязан к `LocalPlayer`, а не к контроллеру: если бы контроллер пересоздался, это было бы
**видно** как смена указателя, а не переход по повисшему.

Измерено в одном процессе, классификация по `FUObjectItem::SerialNumber`:

| Якорь | Живой | Экран смерти | После возрождения | Вердикт |
|---|---|---|---|---|
| World | idx 31078 ser 14729 | то же | то же | **SURVIVED** |
| GameInstance | idx 25993 ser 1365 | то же | то же | **SURVIVED** |
| LocalPlayer | idx 26042 ser 1374 | то же | то же | **SURVIVED** |
| PlayerController | idx 76535 ser 21658 | то же | то же | **SURVIVED** |
| Инвентарь игрока | idx 76537 ser 21659 | то же | то же | **SURVIVED** |
| Pawn | idx 88824 ser 32743 | отсутствует | idx 237092 ser 350768 | **RECREATED** |

Ни слот, ни адрес не переиспользованы. Владение согласовано (`Controller::Pawn ==
AcknowledgedPawn`), цепочка после возрождения полна, независимый оракул готовности — да, все
четыре перекрёстные проверки выполнились и прошли.

**Предсказание было неверным, и это записано.** Ожидалось, что именно **якорь инвентаря** окажется
самым уязвимым при возрождении, потому что `BP_PlayerInventory_C` объявляет
`ClearInventoryOnDeath` и `DeletePlayerSaveOnDeath`. Инвентарь пережил возрождение с
**идентичными** `InternalIndex` и `SerialNumber`: что бы эти флаги ни делали, компонент они не
пересоздают.

**Граница наблюдения названа.** `Pawn` и `AcknowledgedPawn` стали ненулевыми и равными **в
пределах одной 50-мс выборки**. Значит промежуточное состояние, где `Pawn` уже задан, а
`AcknowledgedPawn` ещё нет, длилось меньше интервала опроса. Это предел инструмента, а не
доказательство того, что такого состояния не было.
...
- Что делают `ClearInventoryOnDeath` и `DeletePlayerSaveOnDeath` (объявлены на
  `BP_PlayerInventory_C`) — относится к M6. Измерено лишь то, что сам компонент при возрождении
  **не** пересоздаётся; что происходит с его содержимым — не проверялось.
- Промежуточное состояние возрождения короче 50 мс и этим инструментом не разрешается.
