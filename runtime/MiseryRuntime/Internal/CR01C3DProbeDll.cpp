// CR-01C3D: First real inventory-visible mod item.
//
//   [C3B] spawn -> root -> RowStruct -> materialize -> AddRow into OUR table
//   [C3C] attach the table as a second parent of MasterItemList, publish
//   resolve : BP_SGKFunctions_C::"SGK ItemDetails" must find our definition
//   additem : ONE vanilla BP_MasterInventory_C::AddItem on the real player inventory
//   remove  : BP_MasterInventory_C::RemoveItem on exactly the slot we observed
//   [C3C] detach, zero the spare slot, release the root
//
// Removal is the exact inverse of AddItem and was established before any
// mutation: RemoveItem calls RemoveItemWeight + ChangeItemCount + SetNewSlotItem
// and touches no spawn, drop or world path.
//
// Both Parms buffers are POD end to end (S_InvItem, S_InvSlot and S_InvSlotID
// contain no FString/FText/TArray), so a zeroed buffer is a valid construction
// and needs no destruction. The ONE exception is the resolver's out-param, a
// full S_ItemDetails, which IS constructed with InitializeStruct and destroyed
// with DestroyStruct so nothing leaks.
#include <atomic>
#include <cstdint>
...
// S_InvItem, reflected: ID@0 Amount@8 MasterInventory@16 QuickBindIndex@24
// Rotated@28 UseAmount@32 InUse@36 Durability@40 DecayTime@44, 48 bytes.
...
// S_ItemDetails.DurabilityDetails@1928 -> S_DurabilityDetails.UseDurability@0,
// .DecayDetails@48 -> S_DecayDetails.UseItemDecay@0.
static constexpr int SD_USE_DURABILITY = 1928;
static constexpr int SD_USE_ITEM_DECAY = 1928 + 48;
// AddItem Parms, reflected: Item@0 StackSearch@48 ShowNotifications@49
// RemainingItem@50 RemainingInvItem@56 NewItemSlot@104, ParmsSize 120.
static constexpr int AI_ITEM = 0, AI_STACKSEARCH = 48, AI_SHOWNOTIF = 49,
                     AI_REMAINING = 50, AI_REMAINING_ITEM = 56, AI_NEWSLOT = 104,
                     AI_PARMS = 120;
// RemoveItem Parms: InvSlot@0 RemoveWeight@80 RemoveInvAmount@81 SpecialSlot@82.
static constexpr int RI_SLOT = 0, RI_REMOVEWEIGHT = 80, RI_REMOVEAMOUNT = 81,
                     RI_SPECIAL = 82, RI_PARMS = 83;
// SGK ItemDetails Parms: InvItem@0 WorldContext@48 ItemFound@56 ItemDetails@64.
...
constexpr uint64_t kMagic = 0x4950502D43334400ULL;  // "IPP-C3D\0"
...
#pragma pack(pop)
static_assert(sizeof(C3DIo) == 1216, "C3DIo layout must match the controller");
...
// Our S_InvItem, built from values the controller derived from 1036 live
// vanilla inventory entries -- notably QuickBindIndex = -1, which every single
// one of them uses and which a zeroed struct would get wrong.
...
void JobPopulate(void*) {
    ...
    init(rs, temp, 1);
    // report what InitializeStruct produced for the two gates that decide
    // whether the per-instance Durability/DecayTime fields matter at all
...
void JobAttach(void*) { C3DIo* io = g_io; io->attach_ran = Attach() ? 1u : 2u; }

// Prove the definition resolves through the REAL resolver the inventory uses.
void JobResolve(void*) {
    ...
    *reinterpret_cast<uint64_t*>(parms + SG_WORLDCTX) = io->player_inventory;
    // the out-param is a full S_ItemDetails and owns memory: construct it
    // properly and destroy it afterwards
    init(rs, parms + SG_DETAILS, 1);
    reinterpret_cast<ProcessEventFn>(static_cast<uintptr_t>(io->process_event))(
        reinterpret_cast<void*>(static_cast<uintptr_t>(io->cdo_sgkfunctions)),
        reinterpret_cast<void*>(static_cast<uintptr_t>(io->fn_sgk_itemdetails)), parms);
...
// The one gameplay mutation: exactly one vanilla AddItem.
void JobAddItem(void*) {
    ...
    reinterpret_cast<ProcessEventFn>(static_cast<uintptr_t>(io->process_event))(
        reinterpret_cast<void*>(static_cast<uintptr_t>(io->player_inventory)),
        reinterpret_cast<void*>(static_cast<uintptr_t>(io->fn_additem)), parms);
...
// Remove exactly the slot the controller observed, using the narrow vanilla
// inverse of AddItem.
void JobRemoveItem(void*) {
    C3DIo* io = g_io;
    if (!io->player_inventory || !io->fn_removeitem) { io->err = 100; io->removeitem_ran = 2; return; }
    // the controller must have staged the observed slot, and it must still be
    // the occupied slot carrying OUR name
    ...
    reinterpret_cast<ProcessEventFn>(static_cast<uintptr_t>(io->process_event))(
        reinterpret_cast<void*>(static_cast<uintptr_t>(io->player_inventory)),
        reinterpret_cast<void*>(static_cast<uintptr_t>(io->fn_removeitem)), parms);
    io->removeitem_ran = 1;
}
...
extern "C" __declspec(dllexport) unsigned long Init(void* param) {
    C3DIo* io = static_cast<C3DIo*>(param);
